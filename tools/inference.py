...
def pasre_args():
    ...
    parser.add_argument('--work-dir', help='if specified, the evaluation metric results will be dumped into '
    ...
    parser.add_argument('--show-dir', help='directory where painted iamges will be saved')
   ...
   #add argument path_dir_input_image and path video (@bmhung 9.12.2023)
    parser.add_argument('--input-dir', default='none', help='input directory images')
    parser.add_argument('--video-path', default='none', help='video path to predict')
    parser.add_argument('--out-video', default=True, help='whether output is video')
...
def main():
    ...
    loader_cfg = dict(
        #cfg.gpus will be ignores if distributed
    ...
    if not distributed:
        ...
        #      args.show,
        #      args.show_dir)
        test(model, data_loader, False, args.show, args.show_dir)
    else: # test multi-GPU
        if cfg.data.test.type.startswith('Openlane') or cfg.data.test.type.startswith('Waymo'):
